...
**openHyperEditor** —— 配套的浏览器端工作台（`apps/editor`）：CanvasKit 所见即所得预览（与引擎渲染同一棵 draw 树）、图层/关键帧时间线、schema 自动生成的属性面板、一键调用 HTTP 服务渲染 MP4：

![openHyperEditor](docs/screenshots/editor.png)

内置**插件库**页 —— 每个动效插件都有实时循环预览、自动参数表单和对应的 Scene Graph IR：

![插件库](docs/screenshots/plugin-gallery.png)

以下动效帧由引擎自身渲染（`openhyper still`），每个效果在 IR 中只是一个非破坏性的 `{ type: "plugin" }` 节点：

| | |
| --- | --- |
| ![neon-trace-title](docs/screenshots/neon-trace-title.png) `neon-trace-title` 霓虹描迹标题 | ![hyperspace-warp](docs/screenshots/hyperspace-warp.png) `hyperspace-warp` 超空间跃迁 |
| ![rgb-glitch-shake](docs/screenshots/rgb-glitch-shake.png) `rgb-glitch-shake` RGB 故障抖动 | ![sticker-pop](docs/screenshots/sticker-pop.png) `sticker-pop` 贴纸弹出 |
| ![kinetic-bars](docs/screenshots/kinetic-bars.png) `kinetic-bars` 动能字条 | ![radar-sweep](docs/screenshots/radar-sweep.png) `radar-sweep` 雷达扫描 |
| ![aperture-reveal](docs/screenshots/aperture-reveal.png) `aperture-reveal` 光圈开启 | ![particle-assemble](docs/screenshots/particle-assemble.png) `particle-assemble` 粒子汇聚 |
...
| `neon-trace-title` | 字母沿霓虹光迹逐个点亮，下划线带光点画出，最后整词灌满霓虹填充 | `text`、`size`、`color`、`glow`、`traceMs`、`background` |
| `aperture-reveal` | 相机光圈叶片先合拢，再旋转张开露出后方场景，最后一片让开时标题居中落下 | `blades`、`color`、`holdMs`、`openMs`、`title`、`background` |
| `radar-sweep` | 荧光雷达网格上扫描线旋转并拖出衰减尾迹；光标被扫过的瞬间闪亮并荡出圆环 | `color`、`rings`、`sweepMs`、`blips`、`background` |
| `kinetic-bars` | 色块从左右两侧交替撞入，各自遮罩一行文字，文字随交错缓动依次归位 | `lines`（以 `/` 分隔）、`barColor`、`staggerMs`、`slideMs`、`background` |
| `particle-assemble` | 散落的粒子从随机位置汇聚成环形标记 logo，字标随后在下方淡入 | `count`、`color`、`assembleMs`、`shape`、`title`、`background` |
| `hyperspace-warp` | 星光拖尾从中心径向加速，到达光速时白闪一下，标题随后从超大尺寸砸入 | `streaks`、`accel`、`flashMs`、`title` |
| `velocity-zoom` | CapCut 风格变速开场：画面随节拍猛推变焦并带运动模糊抖动，最后定格到标题 | `text`、`beats`、`zoom`、`shake`、`blur`、`accent` |
| `beat-bounce` | 粗体大字砸入并随每个节拍压扁回弹，带投影——经典卡点歌词字幕风格 | `text`、`bounce`、`beatMs`、`boxColor`/`textColor`、`accent` |
| `rgb-glitch-shake` | 色差故障：RGB 通道分离抖动、故障条撕过标题、数字噪点闪烁——"黑客感"开场 | `text`、`split`、`sliceRate`、`noise`、`color`/`colorB` |
| `sticker-pop` | Emoji/贴纸五彩纸屑带弹性过冲爆开，围绕弹跳的气泡字幕——反应视频式俏皮开场 | `caption`、`stickers`、`pop`、`spin`、`bubbleColor`、`accent` |

最新的十个里，前六个（`neon-trace-title` 到 `hyperspace-warp`）是 `opener` 开场效果，后四个（`velocity-zoom`、`beat-bounce`、`rgb-glitch-shake`、`sticker-pop`，category 为 `tiktok`）组成 TikTok/CapCut 风格套装——节奏感强的短视频卡点开场。它们的完整参数 schema：

**`neon-trace-title`**（默认时长 3400 ms）

| 参数 | 标签 | 类型 | 默认值 | 范围 |
| --- | --- | --- | --- | --- |
| `text` | 标题文字 | string | `"HYPERCORE"` | — |
| `size` | 字号（0 = 自动） | number | `0` | ≥ 0 |
| `color` | 霓虹颜色 | color | `#4f8cff` | — |
| `glow` | 光晕半径（px） | number | `18` | 0–60 |
| `traceMs` | 描迹时长（ms） | number | `2400` | ≥ 200 |
| `background` | 背景色 | color | `#0d1526` | — |

**`aperture-reveal`**（默认时长 3600 ms）

| 参数 | 标签 | 类型 | 默认值 | 范围 |
| --- | --- | --- | --- | --- |
| `blades` | 叶片数 | number | `8` | 3–16 |
| `color` | 强调色 | color | `#4f8cff` | — |
| `holdMs` | 合拢停留（ms） | number | `300` | ≥ 0 |
| `openMs` | 张开时长（ms） | number | `1600` | ≥ 200 |
| `title` | 标题文字 | string | `"APERTURE"` | — |
| `background` | 背景色 | color | `#2a3b66` | — |

**`radar-sweep`**（默认时长 4000 ms）

| 参数 | 标签 | 类型 | 默认值 | 范围 |
| --- | --- | --- | --- | --- |
| `color` | 荧光颜色 | color | `#7dffcf` | — |
| `rings` | 圆环数 | number | `4` | 1–8 |
| `sweepMs` | 扫描周期（ms） | number | `4000` | ≥ 500 |
| `blips` | 光标数 | number | `4` | 0–8 |
| `background` | 背景色 | color | `#060d0a` | — |

**`kinetic-bars`**（默认时长 3600 ms）

| 参数 | 标签 | 类型 | 默认值 | 范围 |
| --- | --- | --- | --- | --- |
| `lines` | 行文本（以 / 分隔） | string | `"MOTION / IN / FRAMES"` | — |
| `barColor` | 色块颜色 | color | `#4f8cff` | — |
| `staggerMs` | 交错间隔（ms） | number | `120` | 0–600 |
| `slideMs` | 滑入时长（ms） | number | `400` | 100–2000 |
| `background` | 背景色 | color | `#0b0f16` | — |

**`particle-assemble`**（默认时长 4000 ms）

| 参数 | 标签 | 类型 | 默认值 | 范围 |
| --- | --- | --- | --- | --- |
| `count` | 粒子数 | number | `90` | 8–240 |
| `color` | 粒子颜色 | color | `#4f8cff` | — |
| `assembleMs` | 汇聚时长（ms） | number | `2800` | ≥ 300 |
| `shape` | 目标形状 | select | `ring` | `ring` / `burst` |
| `title` | 标题文字 | string | `"ASSEMBLE"` | — |
| `background` | 背景色 | color | `#070a12` | — |

**`hyperspace-warp`**（默认时长 3800 ms）

| 参数 | 标签 | 类型 | 默认值 | 范围 |
| --- | --- | --- | --- | --- |
| `streaks` | 拖尾数量 | number | `120` | 20–240 |
| `accel` | 加速指数 | number | `2.0` | 1–4 |
| `flashMs` | 白闪时长（ms） | number | `300` | 0–1000 |
| `title` | 标题文字 | string | `"HYPERSPACE"` | — |

**`velocity-zoom`**（默认时长 2600 ms）

| 参数 | 标签 | 类型 | 默认值 | 范围 |
| --- | --- | --- | --- | --- |
| `text` | 标题文字 | string | `"GO VIRAL"` | — |
| `beats` | 节拍数 | number | `3` | 1–8 |
| `zoom` | 推近倍率 | number | `1.9` | 1.1–3 |
| `shake` | 抖动（px） | number | `14` | 0–40 |
| `blur` | 运动模糊 | number | `0.6` | 0–1 |
| `accent` | 强调色 | color | `#4f8cff` | — |

**`beat-bounce`**（默认时长 2400 ms）

| 参数 | 标签 | 类型 | 默认值 | 范围 |
| --- | --- | --- | --- | --- |
| `text` | 字幕文字 | string | `"LET’S GO"` | — |
| `bounce` | 回弹幅度 | number | `0.7` | 0–1 |
| `beatMs` | 节拍周期（ms） | number | `400` | 120–1200 |
| `boxColor` | 底框颜色 | color | `#000000` | — |
| `textColor` | 文字颜色 | color | `#ffffff` | — |
| `accent` | 强调色 | color | `#4f8cff` | — |

**`rgb-glitch-shake`**（默认时长 2600 ms）

| 参数 | 标签 | 类型 | 默认值 | 范围 |
| --- | --- | --- | --- | --- |
| `text` | 标题文字 | string | `"GLITCH"` | — |
| `split` | 通道分离（px） | number | `16` | 0–48 |
| `sliceRate` | 撕裂条频率 | number | `0.4` | 0–1 |
| `noise` | 噪点强度 | number | `0.3` | 0–1 |
| `color` | 通道 A 颜色 | color | `#ff2e63` | — |
| `colorB` | 通道 B 颜色 | color | `#2effe6` | — |

**`sticker-pop`**（默认时长 3000 ms）

| 参数 | 标签 | 类型 | 默认值 | 范围 |
| --- | --- | --- | --- | --- |
| `caption` | 字幕 | string | `"OMG 😱"` | — |
| `stickers` | 贴纸（空格分隔） | string | `"✨ 🔥 💖 😂 ⭐"` | — |
| `pop` | 过冲幅度 | number | `0.8` | 0–1 |
| `spin` | 环绕漂移 | number | `0.3` | 0–1 |
| `bubbleColor` | 气泡颜色 | color | `#ffffff` | — |
| `accent` | 贴纸强调色 | color | `#ffd166` | — |
...
- 动效插件：`{ type: "plugin" }` IR 节点经 `openhypercore/plugins` 展开为普通图层（`expandComposition`，CLI/服务/编辑器自动执行），参数带可序列化 schema（编辑器自动生成表单）——从舞台幕布到旋转卫星地球仪共 18 个内置插件（见上文章节），并可用 `definePlugin`/`registerPlugin` 自定义。
