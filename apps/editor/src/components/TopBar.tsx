    ...
    <header className="topbar">
      ...
      {inEditor ? (
        <>
          ...
          <button className="icon-btn" title={t("新建")} onClick={onNew}><Icon name="file" size={15} /></button>
          <button className="icon-btn" title={t("打开")} onClick={onOpen}><Icon name="open" size={15} /></button>
          <button className="icon-btn" title={t("保存工程 JSON (⌘S)")} onClick={onSave}><Icon name="save" size={15} /></button>

          <div className="divider" />
          <button className="icon-btn" title={t("把多选图层合并为组 (⌘G)，⇧/⌘ 点选多个图层")} disabled={!canGroup} onClick={onGroup}><Icon name="group" size={15} /></button>
          <button className="icon-btn" title={t("把选中的组拆回独立图层 (⇧⌘G)")} disabled={!canUngroup} onClick={onUngroup}><Icon name="close" size={14} /></button>
        </>
      ) : null}

      <div className="spacer" />
      <span style={{ color: "var(--muted)", fontSize: 12, maxWidth: 240, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{status}</span>
