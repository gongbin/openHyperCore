    ...
    return (
      <aside className="inspector">
        <div className="inspector-scroll">
          <div className="section-title" style={{ marginTop: 4 }}>{t("场景 IR JSON")}</div>
          ...
          <div style={{ color: "var(--faint)", fontSize: 11 }}>{t("这就是渲染服务收到的完整场景描述 —— 编辑会实时生效。")}</div>
        </div>
      </aside>
    );
  }
    ...
    <aside className="inspector">
      <div className="inspector-scroll">
        {layer ? (
          <>
            {selection.length > 1 ? (
              <button className="btn btn-ghost" style={{ alignSelf: "flex-start", padding: "3px 8px", fontSize: 11.5 }}
                onClick={() => onSelect(selection.slice(0, -1))}>{t("← 返回上级（组）")}</button>
            ) : null}
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <input className="input" style={{ width: 110 }} placeholder={t("图层 id")} value={(layer.id as string) ?? ""}
                onChange={(e) => patchLayer({ id: e.target.value || undefined }, "id")} />
            </div>

            <Section title={t("入场 / 出场动画")}>
              <div style={{ color: "var(--faint)", fontSize: 10.5 }}>{t("鼠标划过 = 在画布上试播 · 点击 = 应用")}</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {([["从左入", "→"], ["从右入", "←"], ["从上入", "↓"], ["从下入", "↑"], ["淡入", "◐"],
                  ["弹出", "✺"], ["缩放入", "⤢"], ["左弧入", "⤾"], ["右弧入", "⤿"], ["淡出", "◑"]] as [string, string][]).map(([a, g]) => (
                  <button key={a} className="chip chip-anim" title={t("划过试看 · 点击应用")}
                    onMouseEnter={() => previewAnim(a)} onMouseLeave={endPreviewAnim}
                    onClick={() => applyAnim(a)}><em>{g}</em>{t(a)}</button>
                ))}
                <button className="chip" style={{ color: "var(--danger)" }} onClick={() => applyAnim("清除")}>{t("清除")}</button>
              </div>
              <button className="chip" style={recording ? { borderColor: "var(--danger)", color: "var(--danger)", alignSelf: "flex-start" } : { alignSelf: "flex-start" }}
                title={t("在画布上按住拖动该图层画出运动轨迹，松开自动生成关键帧（从当前播放头开始，按真实拖动节奏）")}
                onClick={onToggleRecord}>
                {recording ? t("● 录制中 — 去画布拖动图层，点此取消") : t("◉ 录制移动路径")}
              </button>
              {(() => {
                const KEY_NAMES: Record<string, string> = { x: t("x 位移"), y: t("y 位移"), scale: t("缩放"), rotate: t("旋转"), opacity: t("不透明度") };
                const tracks = TRANSFORM_KEYS.filter((k) => Array.isArray(trRaw[k]) && (trRaw[k] as Kf[]).length > 0);
                if (!tracks.length) {
                  return <div style={{ color: "var(--faint)", fontSize: 11 }}>{t("此图层还没有动画 — 划过上方按钮试看效果，或点画布上的「✦ 动一动」把它拖到想去的位置。")}</div>;
                }
                return (
                      ...
                      return (
                        <div key={k} style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 11.5, background: "var(--panel-2)", border: "1px solid var(--border)", borderRadius: 7, padding: "4px 8px" }}>
                          ...
                          <span style={{ color: "var(--muted)", flex: 1 }}>
                            {t("{n} 帧", { n: arr.length })} · {Math.round(first.timeMs)}→{Math.round(last.timeMs)}ms · {t("值")} {r2(first.value)}→{r2(last.value)}
                          </span>
                          <button className="kf-btn" style={{ width: 20, height: 20, fontSize: 10 }} title={t("清除该属性的动画（保留末值）")}
                            onClick={() => patchLayer({ transform: { ...trRaw, [k]: last.value } })}>✕</button>
                        </div>
                      );
                    })}
                    <div style={{ color: "var(--faint)", fontSize: 10.5 }}>{t("时间为{scope} · 点时间轴 ◆ 可改时刻与缓动", { scope: layer.type === "group" || layer.type === "plugin" ? t("组内本地时间（0 = 图层开始）") : t("合成全局时间") })}</div>
                  </div>
                );
              })()}
            </Section>
            ...
            <Section title={t("变换")}>
              ...
              <Row>
                <KfNum label={t("缩放")} step={0.05} value={rVal("scale")} animated={animOf("scale")} hasKey={keyOf("scale")} onChange={(v) => editTransform("scale", v)} onToggle={() => toggleKey("scale")} />
                <KfNum label={t("旋转°")} value={rVal("rotate")} animated={animOf("rotate")} hasKey={keyOf("rotate")} onChange={(v) => editTransform("rotate", v)} onToggle={() => toggleKey("rotate")} />
              </Row>
              <KfRange label={t("不透明度")} value={rVal("opacity")} animated={animOf("opacity")} hasKey={keyOf("opacity")} onChange={(v) => editTransform("opacity", v)} onToggle={() => toggleKey("opacity")} />
            </Section>

            {selKfData && selTuple && selKf ? (
              <Section title={t("缓动 — {key} @ {ms}ms", { key: selKf.key, ms: Math.round(selKfData.timeMs) })}>
                <Sel label={t("预设")} value={presetName(selTuple)} options={["custom", ...Object.keys(EASINGS)]}
              ...
              <div style={{ color: "var(--faint)", fontSize: 11 }}>{t("点击时间轴上的 ◆ 关键帧可编辑缓动曲线。")}</div>
            )}

            <Section title={t("时间")}>
              <Row>
                <Num label={t("开始 ms")} value={(layer.startMs as number) ?? 0} onChange={(v) => patchLayer({ startMs: v || undefined }, "startMs")} />
                <Num label={t("结束 ms")} value={(layer.endMs as number) ?? composition.durationMs} onChange={(v) => patchLayer({ endMs: v }, "endMs")} />
              </Row>
            </Section>

            <Section title={t("效果")}>
              <Sel label={t("混合模式")} value={(layer.blendMode as string) ?? "normal"} options={BLEND_MODES}
                onChange={(v) => patchLayer({ blendMode: v === "normal" ? undefined : v })} />
              <Num label={t("模糊")} value={(layer.blur as number) ?? 0} onChange={(v) => patchLayer({ blur: v || undefined }, "blur")} />
            </Section>
          ...
          <div className="empty-hint">{t("未选中图层。")}<br />{t("点击画布或时间轴选中后在此调节。")}</div>
        )}

        <div style={{ flex: 1 }} />
        <Section title={t("合成设置")}>
          <Row>
            <Num label="fps" value={composition.fps} onChange={(v) => patchComposition({ fps: v })} />
            <Num label={t("时长 ms")} value={composition.durationMs} onChange={(v) => patchComposition({ durationMs: v })} />
          </Row>
          <Row>
            <Num label={t("宽")} value={composition.width} onChange={(v) => patchComposition({ width: v })} />
            <Num label={t("高")} value={composition.height} onChange={(v) => patchComposition({ height: v })} />
          </Row>
        </Section>
      </div>
    </aside>
...
function IrPanel({ layer }: { layer: AnyLayer }) {
  ...
  const json = JSON.stringify(layer, (_k, v: unknown) => (typeof v === "string" && v.startsWith("data:") ? t("{head}…(内嵌)", { head: v.slice(0, 48) }) : v), 2);
  return (
...
function AssetPicker({ assets, kind, onPick }: { assets: EditorAsset[]; kind: EditorAsset["kind"]; onPick: (url: string) => void }) {
  ...
  return (
    <Sel label={t("使用已导入素材")} value="" options={["", ...list.map((a) => a.id)]}
      labels={Object.fromEntries([["", t("选择素材…")], ...list.map((a) => [a.id, a.name] as [string, string])])}
...
function srcLabel(src: unknown): string {
  const s = String(src ?? "");
  if (s.startsWith("data:")) return t("内嵌素材（{kb} KB）", { kb: Math.round(s.length * 0.75 / 1024) });
  if (s.startsWith("blob:")) return t("本地素材（仅预览）");
  return s;
}
...
function SrcField({ layer, assets, kind, set }: { layer: AnyLayer; assets: EditorAsset[]; kind: EditorAsset["kind"]; set: (p: Record<string, unknown>) => void }) {
  ...
  return (
    <>
      <Col label={t("来源 src")}>
        <input className="input" value={embedded ? srcLabel(src) : src} readOnly={embedded} placeholder={t("https://… 或从素材库选择")}
...
function FillEditor({ layer, set }: { layer: AnyLayer; set: (p: Record<string, unknown>, tag?: string) => void }) {
  ...
  if (mode === "keyframes") return <div style={{ color: "var(--faint)", fontSize: 11 }}>{t("填充带颜色关键帧 — 在 JSON 中编辑")}</div>;

  return (
    <>
      <Sel label={t("填充")} value={mode} options={["solid", "linear", "radial"]} labels={{ solid: t("纯色"), linear: t("线性渐变"), radial: t("径向渐变") }}
        onChange={(v) => {
      ...
      {mode === "solid" ? (
        <ColorField label={t("颜色")} value={solid} onChange={(v) => set({ fill: v }, "fill")} />
      ) : (
        <>
          <Row>
            <ColorField label={t("起始色")} value={stops[0]?.color ?? "#4d8dff"}
              onChange={(v) => set({ fill: { ...g!, stops: [{ offset: 0, color: v }, stops[1] ?? { offset: 1, color: "#22d3ee" }] } }, "fill")} />
            <ColorField label={t("结束色")} value={stops[stops.length - 1]?.color ?? "#22d3ee"}
              onChange={(v) => set({ fill: { ...g!, stops: [stops[0] ?? { offset: 0, color: "#4d8dff" }, { offset: 1, color: v }] } }, "fill")} />
          </Row>
          {mode === "linear" ? (
            <Sel label={t("方向")} value={gradientDir(g!)} options={["horizontal", "vertical", "diagonal"]} labels={{ horizontal: t("水平"), vertical: t("垂直"), diagonal: t("对角") }}
...
function ShapeProps({ layer, set }: { layer: AnyLayer; set: (p: Record<string, unknown>, tag?: string) => void }) {
  ...
  return (
    <Section title={t("形状")}>
      <Sel label={t("类型")} value={(layer.shape as string) ?? "rect"} options={["rect", "circle", "path"]} labels={{ rect: t("矩形"), circle: t("圆形"), path: t("SVG 路径") }} onChange={(v) => set({ shape: v })} />
      {layer.shape === "circle"
        ? <Num label={t("半径")} value={(layer.radius as number) ?? 0} onChange={(v) => set({ radius: v }, "radius")} />
        : <Row>
            <Num label={t("宽")} value={(layer.width as number) ?? 0} onChange={(v) => set({ width: v }, "w")} />
            <Num label={t("高")} value={(layer.height as number) ?? 0} onChange={(v) => set({ height: v }, "h")} />
          </Row>}
      {isPath ? (
        <Col label={t("路径 d（SVG path 语法）")}>
      ...
      <Row>
        <ColorField label={t("描边")} value={typeof layer.stroke === "string" ? layer.stroke : "#000000"} onChange={(v) => set({ stroke: v }, "stroke")} />
        <Num label={t("描边宽")} value={typeof layer.strokeWidth === "number" ? layer.strokeWidth : 0}
          onChange={(v) => set(v ? { strokeWidth: v } : { strokeWidth: undefined, stroke: undefined }, "strokeW")} />
      </Row>
      {isPath ? (
        <>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            <button className="chip" title={t("路径从头到尾描画出现（需要描边）")}
              onClick={() => set({ trimEnd: [{ timeMs: 0, value: 0 }, { timeMs: 1200, value: 1, easing: [0.2, 0, 0, 1] }] })}>{t("✏️ 描画动画")}</button>
            {trimAnimated ? <button className="chip" style={{ color: "var(--danger)" }} onClick={() => set({ trimStart: undefined, trimEnd: undefined })}>{t("清除描画")}</button> : null}
          </div>
          {trimAnimated ? <div style={{ color: "var(--faint)", fontSize: 11 }}>{t("trimEnd 0→1 关键帧已添加（1.2s 描画）。更细的控制在 JSON 中调。")}</div> : null}
        </>
      ) : null}
    </Section>
  );
...
function TextProps({ layer, caption, set }: { layer: AnyLayer; caption: boolean; set: (p: Record<string, unknown>, tag?: string) => void }) {
  ...
  return (
    <Section title={caption ? t("字幕") : t("文字")}>
      <Col label={t("内容")}>
        <textarea className="input" value={(layer.text as string) ?? ""} rows={2} style={{ resize: "vertical" }}
          onChange={(e) => set({ text: e.target.value }, "text")} />
      </Col>
      <Row>
        <Num label={t("字号")} value={(layer.size as number) ?? 16} onChange={(v) => set({ size: v }, "size")} />
        <Sel label={t("对齐")} value={(layer.align as string) ?? "left"} options={["left", "center", "right"]} labels={{ left: t("左"), center: t("中"), right: t("右") }} onChange={(v) => set({ align: v })} />
      </Row>
      <Row>
        <Num label={t("行高")} step={0.1} value={(layer.lineHeight as number) ?? 1.3} onChange={(v) => set({ lineHeight: v }, "lh")} />
        <ColorField label={t("颜色")} value={color} onChange={(v) => set({ color: v }, "color")} />
      </Row>
      <Num label={t("字距 letterSpacing（px）")} value={(layer.letterSpacing as number) ?? 0} onChange={(v) => set({ letterSpacing: v || undefined }, "lsp")} />
      {caption ? (
        <Row>
          <ColorField label={t("底色")} value={typeof layer.backgroundColor === "string" ? layer.backgroundColor : "#000000"} onChange={(v) => set({ backgroundColor: v }, "bg")} />
          <Num label={t("内边距")} value={(layer.padding as number) ?? 8} onChange={(v) => set({ padding: v }, "pad")} />
        </Row>
      ) : (
        <>
          <Row>
            <ColorField label={t("描边")} value={typeof layer.stroke === "string" ? layer.stroke : "#000000"} onChange={(v) => set({ stroke: v }, "tstroke")} />
            <Num label={t("描边宽")} value={(layer.strokeWidth as number) ?? 0} onChange={(v) => set({ strokeWidth: v || undefined }, "tstrokeW")} />
          </Row>
          <Row>
            <ColorField label={t("阴影色")} value={typeof layer.shadowColor === "string" ? layer.shadowColor : "#000000"} onChange={(v) => set({ shadowColor: v }, "shc")} />
            <Num label={t("阴影模糊")} value={(layer.shadowBlur as number) ?? 0} onChange={(v) => set({ shadowBlur: v || undefined }, "shb")} />
          </Row>
        </>
      )}
      <Num label={t("最大宽度（自动换行，0 = 不限制）")} value={(layer.maxWidth as number) ?? 0} onChange={(v) => set({ maxWidth: v || undefined }, "maxw")} />
      <Col label={t("字体 URL（可选 ttf/otf）")}>
...
function MediaProps({ layer, kind, assets, set }: { layer: AnyLayer; kind: "image"; assets: EditorAsset[]; set: (p: Record<string, unknown>, tag?: string) => void }) {
  return (
    <Section title={t("图片")}>
      ...
      <Sel label={t("适配")} value={(layer.fit as string) ?? "cover"} options={["cover", "contain", "fill"]} labels={{ cover: t("裁切填满"), contain: t("完整显示"), fill: t("拉伸") }} onChange={(v) => set({ fit: v })} />
      <Row>
        <Num label={t("宽（0=原始）")} value={(layer.width as number) ?? 0} onChange={(v) => set({ width: v || undefined }, "w")} />
        <Num label={t("高（0=原始）")} value={(layer.height as number) ?? 0} onChange={(v) => set({ height: v || undefined }, "h")} />
      </Row>
    </Section>
  );
}
...
function VideoProps({ layer, assets, set }: { layer: AnyLayer; assets: EditorAsset[]; set: (p: Record<string, unknown>, tag?: string) => void }) {
  return (
    <Section title={t("视频")}>
      ...
      <Sel label={t("适配")} value={(layer.fit as string) ?? "cover"} options={["cover", "contain", "fill"]} labels={{ cover: t("裁切填满"), contain: t("完整显示"), fill: t("拉伸") }} onChange={(v) => set({ fit: v })} />
      <Row>
        <Num label={t("宽")} value={(layer.width as number) ?? 0} onChange={(v) => set({ width: v || undefined }, "w")} />
        <Num label={t("高")} value={(layer.height as number) ?? 0} onChange={(v) => set({ height: v || undefined }, "h")} />
      </Row>
      <Row>
        <Num label={t("裁入 ms")} value={(layer.trimStartMs as number) ?? 0} onChange={(v) => set({ trimStartMs: v || undefined }, "trimS")} />
        <Num label={t("裁出 ms")} value={(layer.trimEndMs as number) ?? 0} onChange={(v) => set({ trimEndMs: v || undefined }, "trimE")} />
      </Row>
      <Row>
        <Num label={t("倍速")} step={0.1} value={(layer.playbackRate as number) ?? 1} onChange={(v) => set({ playbackRate: v === 1 ? undefined : v }, "rate")} />
        <Num label={t("音量")} step={0.1} value={typeof layer.volume === "number" ? layer.volume : 1} onChange={(v) => set({ volume: v === 1 ? undefined : v }, "vol")} />
      </Row>
      <Toggle label={t("循环播放（需设置裁出）")} checked={Boolean(layer.loop)} onChange={(v) => set({ loop: v || undefined })} />
      <div style={{ color: "var(--faint)", fontSize: 11 }}>{t("视频自带音轨会按上面的裁剪/倍速/音量混入导出的 MP4（音量 0 = 静音）；预览为逐帧抓取，播放可能低于全速。")}</div>
    </Section>
  );
}
...
function AudioProps({ layer, assets, set }: { layer: AnyLayer; assets: EditorAsset[]; set: (p: Record<string, unknown>, tag?: string) => void }) {
  return (
    <Section title={t("音频")}>
      ...
      <Num label={t("音量")} step={0.1} value={typeof layer.volume === "number" ? layer.volume : 1} onChange={(v) => set({ volume: v === 1 ? undefined : v }, "vol")} />
      <Row>
        <Num label={t("淡入 ms")} value={(layer.fadeInMs as number) ?? 0} onChange={(v) => set({ fadeInMs: v || undefined }, "fi")} />
        <Num label={t("淡出 ms")} value={(layer.fadeOutMs as number) ?? 0} onChange={(v) => set({ fadeOutMs: v || undefined }, "fo")} />
      </Row>
      <div style={{ color: "var(--faint)", fontSize: 11 }}>{t("音频在导出 MP4 时混入；编辑器预览暂不出声。")}</div>
    </Section>
  );
}
    ...
    <Section title={t("组")}>
      {children.length ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
          <span className="field-label">{t("子图层（点击编辑其填充/属性，画布双击也可直达）")}</span>
          {children.map((c, i) => (
            <button key={i} className="layer-row" style={{ width: "100%", textAlign: "left", background: "var(--panel-2)", border: "1px solid var(--border)" }}
              ...
              <span style={{ color: "var(--faint)", fontSize: 10.5 }}>{c.type === "shape" && c.fill && typeof c.fill === "object" && !Array.isArray(c.fill) ? t("渐变") : ""}</span>
            </button>
          ))}
        </div>
      ) : null}
      {clip ? (
        <div style={{ color: "var(--muted)", fontSize: 11.5, display: "flex", alignItems: "center", gap: 6 }}>
          {t("裁剪：")}{String(clip.type)} {clip.width ? `${String(clip.width)}×${String(clip.height)}` : ""}{clip.radius ? t(" · 圆角 {r}", { r: String(clip.radius) }) : ""}
          <button className="kf-btn" style={{ width: 20, height: 20, fontSize: 10 }} title={t("移除裁剪")} onClick={() => set({ clip: undefined })}>✕</button>
        </div>
      ) : null}
      <Toggle label={t("静态内容栅格缓存（内容逐帧变化时关闭）")} checked={Boolean(layer.cache)} onChange={(v) => set({ cache: v || undefined })} />
      <Sel label={t("揭示动画 reveal")} value={(reveal?.type as string) ?? "none"} options={["none", "wipe", "clock"]} labels={{ none: t("无"), wipe: t("扫掠"), clock: t("时钟") }}
        onChange={(v) => {
      ...
      {reveal ? (
        <>
          <Row>
            <Num label={t("宽")} value={(reveal.width as number) ?? 0} onChange={(v) => set({ reveal: { ...reveal, width: v } }, "rvw")} />
            <Num label={t("高")} value={(reveal.height as number) ?? 0} onChange={(v) => set({ reveal: { ...reveal, height: v } }, "rvh")} />
          </Row>
          {reveal.type === "wipe" ? (
            <Sel label={t("方向")} value={(reveal.direction as string) ?? "from-left"} options={["from-left", "from-right", "from-top", "from-bottom"]}
              labels={{ "from-left": t("从左"), "from-right": t("从右"), "from-top": t("从上"), "from-bottom": t("从下") }}
              onChange={(v) => set({ reveal: { ...reveal, direction: v } })} />
          ) : null}
        </>
      ) : null}
      <div style={{ color: "var(--faint)", fontSize: 11 }}>{t("子图层在图层树中选择编辑；组内时间从组的开始时刻起算。")}</div>
    </Section>
  );
}
...
function PluginProps({ layer, plugins, assets, set }: { layer: AnyLayer; plugins: PluginDefinition[]; assets: EditorAsset[]; set: (p: Record<string, unknown>, tag?: string) => void }) {
  ...
  if (!def) return <div style={{ color: "var(--danger)", fontSize: 11 }}>{t("未知插件: {name}", { name: String(layer.plugin) })}</div>;
  const params = (layer.params as Record<string, unknown>) ?? {};
  const put = (key: string, v: unknown, tag?: string) => set({ params: { ...params, [key]: v } }, tag);
  return (
    <Section title={t("特效 — {name}", { name: def.displayName ?? def.name })}>
...
export function ParamField({ name, spec, value, assets, onChange }: { name: string; spec: ParamSpec; value: unknown; assets: EditorAsset[]; onChange: (v: unknown, tag?: string) => void }) {
  ...
  switch (spec.type) {
    ...
    case "latlng": {
      ...
      return (
        <Row>
          <Num label={t("{label} · 纬度", { label })} step={0.01} value={lat} onChange={(v) => onChange([v, lng], `p-${name}`)} />
          <Num label={t("经度")} step={0.01} value={lng} onChange={(v) => onChange([lat, v], `p-${name}`)} />
        </Row>
      );
    }
    case "string":
